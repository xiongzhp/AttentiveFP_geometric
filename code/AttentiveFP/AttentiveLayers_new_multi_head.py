...
class graphAttention(nn.Module):
    ...
    def __init__(self, bond_dim, fingerprint_dim, p_dropout, head = 3):
        ...
        self.align = nn.ModuleList([nn.Linear(fingerprint_dim*2,1) for _ in range(head)])
        self.attend = nn.ModuleList([LinearBn(fingerprint_dim, fingerprint_dim) for _ in range(head)])
        self.gru = nn.GRUCell(fingerprint_dim*3, fingerprint_dim)
...
#         neighbor = self.encoder(torch.cat([bond, neighbor_atom],-1))  # concat
        ...
        for head in range(3):
            ...
            align_score = F.leaky_relu(self.align[head](self.dropout(feature_align)))

            attention_weight = self.softmax(align_score, bond_index[0], num=num_atom)

            context = scatter_('add', torch.mul(attention_weight, self.attend[head](self.dropout(neighbor))), \
                               bond_index[0], dim_size=num_atom)

            contexts.append(F.elu(context))

        update = self.gru(torch.cat(contexts,-1), atom)
...
def null_collate(batch):
    ...
    label = torch.from_numpy(np.concatenate(label)).float()
