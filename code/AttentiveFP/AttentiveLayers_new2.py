...
class LinearBn(nn.Module):
    def __init__(self, in_channel, out_channel, act=None):
        ...
        self.bn = nn.BatchNorm1d(out_channel, eps=1e-05, momentum=0.1, affine=True)
        self.act = act
...
class graphAttention(nn.Module):
    ...
    def forward(self, atom, bond_index):
        ...
        neighbor = atom[bond_index[1]]  # 获得所有边的第二个节点的特征 
...
class Fingerprint(torch.nn.Module):
    ...
    def __init__(self, num_target, fingerprint_dim, K=3, T=3, p_dropout=0.2, atom_dim=39, bond_dim=10):
        ...
        self.encoder = nn.Sequential(
            LinearBn(bond_dim + atom_dim, fingerprint_dim),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, atom, bond, bond_index, mol_index):
        ...
        for k in range(self.K):
            atom = self.propagate[k](atom, bond_index)
            atoms.append(atom)
...
class Fingerprint_viz(torch.nn.Module):
    def softmax(self, x, index, num=None):
        ...
        x = x / (scatter_add(x, index, dim=0, dim_size=num)[index] + 1e-8)
        return x
    def __init__(self, num_target, fingerprint_dim, K=2, T=2, p_dropout=0.2, atom_dim=39, bond_dim=10):
