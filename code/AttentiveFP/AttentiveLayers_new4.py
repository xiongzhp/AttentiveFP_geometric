...
class graphAttention(nn.Module):
    ...
    def __init__(self, bond_dim, fingerprint_dim, p_dropout):
        ...
        self.encoder = nn.Sequential(
            LinearBn(bond_dim, fingerprint_dim),
    ...
    def forward(self, atom, bond_index, bond):
        ...
        bond = self.encoder(bond)  # 将边特征转变为节点特征大小  (12,128,128)

        neighbor = neighbor_atom * bond
...
#         self.superGather = superatomAttention(fingerprint_dim, p_dropout)
...
#             superatoms.append(superatom)
#         superatom = torch.mean(torch.stack(superatoms), 0)
